...
## are inhereted.
.initializeProtoClasses <- function(){
...
###_ CLASS REPRESENTATIONS:
    ...
    ## classDef. each new protoContext class generates new classDef with
    ## it's default context.p
...
setCellClass <- function(Class,
    ...
    setClass(Class, contains = contains,
             where = where, ...)
    classDef <- new("cellClassRepresentation", getClassDef(Class, where = where),
...
###_ + envProtoClass
.insertSpecial_envProtoClass <- function(objEnv){
...
.initializeRoot_envProtoClass <- function(.Object,  ##TODO: !! get the "pure" initialization functionality into separate slot "initialize" in class definition!!
                                          ...
                                          type = "--",
                                          ...){ #... not used here pu
    ## initialize the basic functionality for root of evnProtoClass
    ...
    .initMethods(list(
                 ...
                 where = objEnv)
    .insertSpecial_envProtoClass(objEnv)
    objEnv[[".prototype"]] <- NULL
    objEnv[[".self"]] <- .Object
    objEnv[[".root"]] <- .Object
    ## FIELDS:
...
.initialize_envProtoClass <- function(.Object,
    ...
    objEnv[[".self"]] <- .Object
    .insertSpecial_envProtoClass(objEnv)
    .setField(.Object, "type", type) ## type is already defined in the root.
    ## USER supplied INITS
    if(changeCallEnv){
...
.initialize_protoContext <- function(.Object,
                                     ...
                                     ...){
    ## PROTOTYPE must be a valid envProtoObject
...
### note: cannot create the root cell!! supplying * as TYPE produces the cell *.*
    ...
    if(!is(context, tCls <- getClassDef(class(.Object))@contextClass))
        stop(gettextf("Attempt to create an object of class %s in a homeContext of class %s which does not extend cells default context class %s",
                      class(.Object), class(homeContext), tCls))
    type <- as.character(type)
    ## GET THE CELL from the container depending whether the context is default or not
    ...
    if(is.null(the_cell)){
        ...
        ## REDIRECT parents of containers   ## note:  no problems with cloning * cell, never occurs; it's just not allowed to create with new interface: )
...
.makeNames <- function(names){
    ## forms must have valid objects (used in initialization for "protoForm")
    if(length(names)){
...
.clone_envProtoClass <- function(x, exclude_names = c(), ...){
    ...
    .insertSpecial_envProtoClass(yEnv)
